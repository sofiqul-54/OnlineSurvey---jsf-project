...
public class TopicDAO {
    ...
    public static boolean add(String uname, String title) {
        ...
        try {
            ...
            ps = con.prepareStatement("insert into topics(topictitle, addedon, uname) values(?,now(),?)");
    ...
    public static boolean addQuestion(Question q) {
        ...
        try {
            con = Database.getConnection();
            ps = con.prepareStatement("insert into questions(questiontext, opt1, opt2, opt3, topicid) values(?,?,?,?,?)");
    ...
    public static ArrayList<Topic> getTopics() {
        try {
            ...
            while (rs.next()) {
                ...
                System.out.println(rs.getString("topictitle"));
            }
            rs.close();
            return al;
        } catch (Exception e) {
        ...
        try {
            ...
            while (questions.next()) {
                ...
                PreparedStatement psa = con.prepareStatement(
                        "select sum(CASE WHEN answer = 1 THEN answer ELSE 0 END) AS opt1count, sum(CASE WHEN answer = 2 THEN answer ELSE 0 END) AS opt2count, sum(CASE WHEN answer = 3 THEN answer ELSE 0 END) AS opt3count from answers_details where questionid = ?");
    ...
    public static ArrayList<Question> getQuestions(String topicid) {
        try {
            System.out.println(topicid+" bbbbbbbbbbb");
            Connection con = Database.getConnection();
    ...
    public static boolean storeSurveyResults(String topicid, ArrayList<Question> questions) {
        ...
        try {
            con = Database.getConnection();
            con.setAutoCommit(false);
            ps = con.prepareStatement("insert into answers_master(topicid, takenon) values(?,now())");
            ps.setString(1, topicid);
            ps.executeUpdate();

            ps = con.prepareStatement("insert into answers_details(questionid, answer) values(?,?)");
